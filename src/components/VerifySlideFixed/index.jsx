...
import { getPicture, reqCheck } from '@s/myApi/sysUserLoginController';
import { Component } from 'react';
import { CSSTransition } from 'react-transition-group';
import { aesEncrypt } from './ase.js';
import defaultImg from './checkTips.jpg';
import './index.less';
...
class VerifySlideFixed extends Component {
  ...
  handleResize() {
    this.setState({ t: 1920 / screen.availWidth });
  }
  ...
  refresh = () => {
    ...
    this.setState({
      ...
      leftBarWidth: '',
      text: '向右滑动完成验证',
      moveBlockBackgroundColor: '#696969',
  ...
  move = (e) => {
    ...
    if (this.state.status && this.state.isEnd == false) {
      ...
      if (Number.isNaN(this.state.startLeft)) {
        this.state.startLeft = Math.floor(x - this.state.barAreaLeft);
      }
      ...
      if (
        move_block_left >=
        this.state.barAreaOffsetWidth - parseInt(parseInt(this.state.blockSize.width) / 2) - 2
      ) {
        move_block_left =
          this.state.barAreaOffsetWidth - parseInt(parseInt(this.state.blockSize.width) / 2) - 2;
      }
      ...
      if (
        Number(move_block_left - this.state.startLeft) < 0 ||
        Number.isNaN(move_block_left - this.state.startLeft)
      ) {
        this.state.moveBlockLeft = 0;
        this.state.leftBarWidth = 0;
      }
      this.setState({
    ...
    if (this.state.status && this.state.isEnd == false) {
      ...
      if (Number.isNaN(moveLeftDistance)) {
        moveLeftDistance = 0;
      }
      let data = {
        captchaType: this.state.captchaType,
  ...
  render() {
    ...
    return (
      // 蒙层
      <div className="mask" style={{ display: isSlideShow ? 'block' : 'none', zoom: this.state.t }}>
